    ...
    return (
        <div className="flex flex-col h-full bg-pm-blue text-white shadow-xl">
            ...
            <nav className="flex-1 py-4 space-y-1 px-3">
                ...
                {/* Separador visual para área admin */}
                {isAdmin && (
                    <div className="pt-3 mt-3 border-t border-blue-900/50">
                        <p className="text-xs text-blue-400/60 uppercase tracking-wider px-3 pb-2 font-semibold">Administração</p>
                        {adminItems.map((item) => (
                            <Link
                                key={`admin-${item.href}`}
                                href={item.href}
                                className="flex items-center px-3 py-2.5 rounded-md hover:bg-blue-800 transition-colors text-blue-200 hover:text-white group"
                            >
                                <item.icon className="h-5 w-5 mr-3 flex-shrink-0" />
                                <span className="font-medium text-sm">{item.title}</span>
                            </Link>
